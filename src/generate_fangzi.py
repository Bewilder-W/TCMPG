...
def generate_p():
    ...
    for i in range(sheet.max_row):
        ...
        for h in herb_total:
            ...
            if herb not in herb_count.keys():
                herb_count[herb] = 1
            else:
                herb_count[herb] += 1
        ...
        for s in indication:
            ...
            # data_p_i.append(1)
            if s not in indicition_phrase.keys():
                indicition_phrase[s] = 1
            else:
                indicition_phrase[s] += 1

    outpath2 = r'./data/症状对应药方数量.txt'
    outpath3 = r'./data/药材对应药方数量.txt'

    with open(outpath2, 'w', encoding='utf-8') as file:
        idx = 1
        for i in indicition_phrase.keys():
            file.write(str(idx) + '\t' + str(i) + '\t' + str(indicition_phrase[i]))
            file.write('\n')
            idx += 1

    with open(outpath3, 'w', encoding='utf-8') as file:
        idx = 1
        for i in herb_count.keys():
            file.write(str(idx) + '\t' + str(i) + '\t' + str(herb_count[i]))
            file.write('\n')
            idx += 1
